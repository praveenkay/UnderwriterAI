...
`;
  ...
  try {
    ...
    if (strategy.shouldChunk) {
      // For massive documents, use very small chunks
      const safeChunkSize = Math.min(3000, strategy.chunkSize!); // Max 3K tokens per chunk for safety
      const chunks = tokenManager.chunkContent(chatContent, safeChunkSize);
      console.log(`Processing ${chunks.length} small chunks for chat history rule extraction`);

      for (let i = 0; i < chunks.length; i++) {
        const chunk = chunks[i];
        
        // Final safety check - ensure chunk is really small
        const actualChunkTokens = tokenManager.estimateTokens(chunk);
        if (actualChunkTokens > 5000) {
          console.log(`Chunk ${i + 1} still too large (${actualChunkTokens} tokens), skipping...`);
          continue;
        }
        ...
        try {
          ...
          console.log(`Extracted ${chunkRules.length} rules from chunk ${i + 1}`);
        } catch (chunkError) {
